...
function validarCPF(campo) {
    ...
    erro = new String;
    if (cpf.length != 0) { //se o campo cpf for diferente de 0 passa para o pr�ximo if.
...
//Bloquea a digita��o de n�meros no campo
...
/*##############################################################################
 *         BUSCAR CID PELA CHANGE DA DEFICI�NCIA
 *##############################################################################*/
$(function ($) {
...
function ValidarCNPJ(Cnpj) {
    ...
    if (cnpj.length != 0) { //se campo cnpj for diferente de 0 entra no pr�ximo if
...
function confirmarDeleteFormacao() {
    ...
    if (ids) {
        var flag;
        flag = window.confirm("Voc� deseja apagar esta forma��o?");
        if (flag) {
            document.deletarFormacao.submit();
        }
    } else {
        alert("Selecione no m�nimo uma forma��o!");
    }
}
...
function confirmarDeleteQualificacao() {
    ...
    if (ids) {
        var flag;
        flag = window.confirm("Voc� deseja apagar esta qualifica��o?");
        if (flag) {
            document.deletarQualificacao.submit();
        }
    } else {
        alert("Selecione no m�nimo uma qualifica��o!");
    }
}
...
function confirmarDeleteExperiencia() {
    ...
    if (ids) {
        var flag;
        flag = window.confirm("Voc� deseja apagar esta experi�ncia?");
        if (flag) {
            document.deletarExperiencia.submit();
        }
    } else {
        alert("Selecione no m�nimo uma experi�ncia!");
    }
}
...
//m�scara das inscri��es estaduais de todos os estados
$(function ($) {
    $('#id_estado').change(function () {
        var uf = $('#id_estado').val();
...
function verificarSelectEnc(vem_do, id, vc, status, id_vaga, id_interno) {
    if (document.getElementById('id_motivoEnc_' + id).value === 'Selecione') {
        ...
        if (document.getElementById('ds_motivoEnc_' + id).value === '') {
            alert("Informe uma descri��o de motivo!");
            return false;
        }
    }
...
// ########## PARA ABA COM COM CANDIDATOS PR�-SELECIONADOS ##########
...
//Fun��o que mantem atualizado a quantidade dos candidatos na sess�o para mostrar nos contadores
    ...
    $.post("../controle/ControleVaga.php?op=verificarVaga", {id_vaga: id_vaga}, function (retorno) {
        if (retorno > 0) {
            $.post("../controle/ControleVaga.php?op=registrar", {id_vagacandidato: vc, ao_status: status, vem_do: vem_do, id_interno: id_interno}, function (retorno) {
                ...
                //pego o valor do campo hidden que verifica quantos encaminhados tem na p�gina e diminuio 1 at� chegar em 0 e liberar a pagina��o.
                ...
                if (status === 'P') {
                    ...
                    $('#pre_t_' + vc).html("Candidato pr�-selecionado");
                    $('#pre_e_' + vc).html("Candidato pr�-selecionado");
            ...
            alert('N�o � poss�vel fazer novas contrata��es. Favor alterar a quantidade de vagas!');
        }
...
//##################ATUALIZA QUANTIDADE DE CANDIDATOS NO TOPO DA P�GINA######################
...
//###########ATUALIZA QUANTIDADE DE CANDIDATOS ENCAMINHADOS NO TOPO DA P�GINA###############
...
//###########ATUALIZA QUANTIDADE DE CANDIDATOS COM BAIXAS AUTOM�TICAS NO TOPO DA P�GINA###############
...
//###########ATUALIZA QUANTIDADE DE CANDIDATOS PR�-SELECIONADOS NO TOPO DA P�GINA###############
...
//###########ATUALIZA QUANTIDADE DE CANDIDATOS CONTRATADOS NO TOPO DA P�GINA###############
...
//###########ATUALIZA QUANTIDADE DE CANDIDATOS DISPENSADOS NO TOPO DA P�GINA###############
...
//########### PARA ABA DE CANDIDATOS COM BAIXAS AUTOM�TICAS #############
...
//########### PARA ABA DE CANDIDATOS PR�-SELECIONADOS ###################
...
function marcaDivulgar() {
    ...
    if (document.getElementById('ao_exibenome').checked === true) {
        //o value vem com N, quando marcado recebe S caso contr�rio N
...
function mostrarProfissao() {
    //chamar essa fun��o no onchange do select.

    //Pega o valor selecionado no select e toca numa vari�vel
...
function inativarVaga() {
    ...
    var pergunta = "VOC� TEM CERTEZA QUE DESEJA INATIVAR A VAGA?";
    pergunta += "\n\n";
    pergunta += "*Inativando a vaga ela n�o receber� mais encaminhamentos e voc� n�o poder� alter�-la novamente, ";
    pergunta += "apenas cadastrando uma nova vaga.";

    if (confirm(pergunta)) {
        //altero o hidden que est� S para N
        document.getElementById('ao_ativo').value = 'N';

        //submeto o formul�rio para inativar a vaga
...
function atualizaListaPreSelecionados(id_vaga) {
    ...
    $("#tabela_pre_selecionados_div").html("<table width='100%' class='tabela_encaminhados'><tr class='table_formacao_cab'><td>&nbsp;</td></tr><tr class='table_formacao_row'><td style='height: 300px;'><center>Atualizando candidatos pr�-selecionados...<br /><br /><i class='fa fa-spinner fa-pulse fa-3x fa-fw font-blue01'></i></center></td></tr></table>");
    $("#tabela_pre_selecionados_div_pesq").html("<table width='100%' class='tabela_encaminhados'><tr class='table_formacao_cab'><td>&nbsp;</td></tr><tr class='table_formacao_row'><td style='height: 300px;'><center>Atualizando candidatos pr�-selecionados...<br /><br /><i class='fa fa-spinner fa-pulse fa-3x fa-fw font-blue01'></i></center></td></tr></table>");
...
function escondeCNH() {
    //esconde o cnh a idade na vaga seja escolhida a op��o at� 16 ou at� 18, caso contr�rio mostra.
    ...
    if (document.getElementById('ds_idade').value === "16") {
        ...
        //Seleciona o index 0, onde est� marcado no select como vazio.
...
function verifica_vazio_profissoes() {
    if (document.getElementById('profissoes').value === "") {
        alert("Campo em branco, escolha uma ou mais profiss�es para concorrer as vagas do banco de oportunidades");
        return false;
    }
}
