/*##############################################################################
 *         VALIDAÔøΩÔøΩO DE CPF
 *##############################################################################*/
$(function(){
  if(document.getElementById('null_salario').value === "S"){
    document.getElementById('nr_sal').style.display = 'none';
  }
});

$(function(){
  if(document.getElementById('nr_salario').value === "0,00" || document.getElementById('nr_salario').value === ""){
    document.getElementById('nr_sal').style.display = 'none';
    document.getElementById('nr_sal').checked = true;
  }
});
function validarCPF(campo) {
    ...
    erro = new String;
    if (cpf.length != 0) { //se o campo cpf for diferente de 0 passa para o prÔøΩximo if.
        if (cpf.length < 11) { //se quantidade de nÔøΩmeros for menor que 11 da o erro.
...
/*##############################################################################
 *         VERIFICA SE A DATA DIGITADA ÔøΩ VÔøΩLIDA
 *##############################################################################*/
/**
...
/*##############################################################################
 *         VALIDAÔøΩÔøΩO PARA PERMITIR APENAS NÔøΩMERO NO CAMPO
 *##############################################################################*/

/**
 * Permite nÔøΩmeros + ponto + virgula
 *
 * CÔøΩdigos ASCII
...
//Bloquea a digitaÔøΩÔøΩo de nÔøΩmeros no campo
...
/*##############################################################################
 *         BUSCAR CID PELA CHANGE DA DEFICIÔøΩNCIA
 *##############################################################################*/
$(function ($) {
...
function ValidarCNPJ(Cnpj) {
    ...
    if (cnpj.length != 0) { //se campo cnpj for diferente de 0 entra no prÔøΩximo if
...
function confirmarDeleteFormacao() {
    ...
    if (ids) {
        var flag;
        flag = window.confirm("VocÍ deseja apagar esta formaÁ„o?");
        if (flag) {
            document.deletarFormacao.submit();
        }
    } else {
        alert("Selecione no mÌnimo uma formaÁ„o!");
    }
}
...
function confirmarDeleteQualificacao() {
    ...
    if (ids) {
        var flag;
        flag = window.confirm("VocÍ deseja apagar esta qualificaÁ„o?");
        if (flag) {
            document.deletarQualificacao.submit();
        }
    } else {
        alert("Selecione no mÌnimo uma qualificaÁ„o!");
    }
}
...
function confirmarDeleteExperiencia() {
    ...
    if (ids) {
        var flag;
        flag = window.confirm("VocÍ deseja apagar esta experiÍncia?");
        if (flag) {
            document.deletarExperiencia.submit();
        }
    } else {
        alert("Selecione no mÌnimo uma experiÍncia!");
    }
}
...
//mÔøΩscara dt_fundacao
$(function ($) {
    $(document).ready(function () {
        $("#dt_fundacao").mask("99/99/9999");
    });
});
//mÔøΩscara nascimento
$(function ($) {
    $(document).ready(function () {
...
//mÔøΩscara das inscriÔøΩÔøΩes estaduais de todos os estados
$(function ($) {
    $('#id_estado').change(function () {
        var uf = $('#id_estado').val();
...
function verificarSelectEnc(vem_do, id, vc, status, id_vaga, id_interno) {
    if (document.getElementById('id_motivoEnc_' + id).value === 'Selecione') {
        ...
        if (document.getElementById('ds_motivoEnc_' + id).value === '') {
            alert("Informe uma descriÁ„o de motivo!");
            return false;
        }
    }
...
// ########## PARA ABA COM COM CANDIDATOS PRÔøΩ-SELECIONADOS ##########
...
//FunÔøΩÔøΩo que mantem atualizado a quantidade dos candidatos na sessÔøΩo para mostrar nos contadores
...
function chamarEdicaoVaga(idvaga) {
    $("#linha_vaga_" + idvaga).html('<td colspan="8"><center>Carregando vaga...<br /><br /><i class="fa fa-spinner fa-pulse fa-3x fa-fw font-blue01"></i></center></td>');
    ...
    $.post("../controle/ControleVaga.php?op=verificarVaga", {id_vaga: id_vaga}, function (retorno) {
        if (retorno > 0) {
            $.post("../controle/ControleVaga.php?op=registrar", {id_vagacandidato: vc, ao_status: status, vem_do: vem_do, id_interno: id_interno}, function (retorno) {
                ...
                //pego o valor do campo hidden que verifica quantos encaminhados tem na pÔøΩgina e diminuio 1 atÔøΩ chegar em 0 e liberar a paginaÔøΩÔøΩo.
                ...
                if (status === 'P') {
                    ...
                    $('#pre_t_' + vc).html("Candidato prÈ-selecionado");
                    $('#pre_e_' + vc).html("Candidato prÈ-selecionado");
            ...
            alert('N„o È possÌvel fazer novas contrataÁıes. Favor alterar a quantidade de vagas!');
        }
...
//##################ATUALIZA QUANTIDADE DE CANDIDATOS NO TOPO DA PÔøΩGINA######################
...
//###########ATUALIZA QUANTIDADE DE CANDIDATOS ENCAMINHADOS NO TOPO DA PÔøΩGINA###############
...
//###########ATUALIZA QUANTIDADE DE CANDIDATOS COM BAIXAS AUTOMÔøΩTICAS NO TOPO DA PÔøΩGINA###############
...
//###########ATUALIZA QUANTIDADE DE CANDIDATOS PRÔøΩ-SELECIONADOS NO TOPO DA PÔøΩGINA###############
...
//###########ATUALIZA QUANTIDADE DE CANDIDATOS CONTRATADOS NO TOPO DA PÔøΩGINA###############
...
//###########ATUALIZA QUANTIDADE DE CANDIDATOS DISPENSADOS NO TOPO DA PÔøΩGINA###############
...
//########### PARA ABA DE CANDIDATOS COM BAIXAS AUTOMÔøΩTICAS #############
...
//########### PARA ABA DE CANDIDATOS PRÔøΩ-SELECIONADOS ###################
...
function marcaAdicional(id) {
    if (document.getElementById('ao_adicional_' + id).checked === true) {
        $('#ladicional_' + id).css('color', '#119d93');
    } else {
...
function marcaBeneficio(id) {
    if (document.getElementById('ao_beneficio_' + id).checked === true) {
        $('#lbeneficio_' + id).css('color', '#119d93');
    } else {
...
function marcaFormacao(id) {
    if (document.getElementById('ao_formacao_' + id).checked === true) {
        $('#lformacao_' + id).css('color', '#119d93');
    } else {
...
function marcaDivulgar() {
    ...
    if (document.getElementById('ao_exibenome').checked === true) {
        //o value vem com N, quando marcado recebe S caso contrÔøΩrio N
        document.getElementById('ao_exibenome').value = "S";
        $('#lnm_empresa').css('color', '#119d93');
    } else {
    ...
    //SAL√ÅRIO
    if (document.getElementById('null_salario').checked === true) {
        //o value vem com N, quando marcado recebe S caso contrÔøΩrio N
        document.getElementById('null_salario').value = "S";
        $('#null_sal').css('color', '#119d93');
        $("#nr_sal").hide();

    } else if(document.getElementById('null_salario').checked === false) {

          document.getElementById('null_salario').value = "N";
          $("#nr_sal").show();

       $('#null_sal').css('color', '#666666');
    }
    //TELEFONE DA EMPRESA
    if (document.getElementById('ao_exibetelefone').checked === true) {
        //o value vem com N, quando marcado recebe S caso contrÔøΩrio N
        document.getElementById('ao_exibetelefone').value = "S";
        $('#lnm_empresatelefone').css('color', '#119d93');
    } else {
    ...
    if (document.getElementById('ao_exibeemail').checked === true) {
        ...
        $('#lnm_empresaemail').css('color', '#119d93');
    } else {
...
function editalDivulgar(){
  //SAL√ÅRIO
  if (document.getElementById('ed_salario').checked === true) {
      //o value vem com N, quando marcado recebe S caso contrÔøΩrio N
      document.getElementById('ed_salario').value = "S";
      $('#ed_sal').css('color', '#119d93');
      $("#nr_sal").hide();

  } else if(document.getElementById('ed_salario').checked === false) {
        document.getElementById('ed_salario').value = "N";
        $("#nr_sal").show();

     $('#ed_sal').css('color', '#666666');
  }
}

function mostrarProfissao() {
    //chamar essa funÔøΩÔøΩo no onchange do select.

    //Pega o valor selecionado no select e toca numa variÔøΩvel
...
function inativarVaga() {
    ...
    var pergunta = "VocÍ TEM CERTEZA QUE DESEJA INATIVAR A VAGA?";
    pergunta += "\n\n";
    pergunta += "*Inativando a vaga ela n„o receber· mais encaminhamentos e VocÍ n„o poder· alter·-la novamente, ";
    pergunta += "apenas cadastrando uma nova vaga.";

    if (confirm(pergunta)) {
        //altero o hidden que estÔøΩ S para N
        document.getElementById('ao_ativo').value = 'N';

        //submeto o formulÔøΩrio para inativar a vaga
...
function atualizaListaTodos(id_vaga) {
    ...
    $("#tabela_todos_div").html("<table width='100%' class='tabela_encaminhados'><tr class='table_formacao_cab'><td>&nbsp;</td></tr><tr class='table_formacao_row'><td style='height: 300px;'><center>Atualizando candidatos...<br /><br /><i class='fa fa-spinner fa-pulse fa-3x fa-fw font-blue01'></i></center></td></tr></table>");
    $("#tabela_todos_div_pesq").html("<table width='100%' class='tabela_encaminhados'><tr class='table_formacao_cab'><td>&nbsp;</td></tr><tr class='table_formacao_row'><td style='height: 300px;'><center>Atualizando candidatos...<br /><br /><i class='fa fa-spinner fa-pulse fa-3x fa-fw font-blue01'></i></center></td></tr></table>");
...
function atualizaListaEncaminhados(id_vaga) {
    ...
    $("#tabela_encaminhados_div").html("<table width='100%' class='tabela_encaminhados'><tr class='table_formacao_cab'><td>&nbsp;</td></tr><tr class='table_formacao_row'><td style='height: 300px;'><center>Atualizando candidatos encaminhados...<br /><br /><i class='fa fa-spinner fa-pulse fa-3x fa-fw font-blue01'></i></center></td></tr></table>");
    $("#tabela_encaminhados_div_pesq").html("<table width='100%' class='tabela_encaminhados'><tr class='table_formacao_cab'><td>&nbsp;</td></tr><tr class='table_formacao_row'><td style='height: 300px;'><center>Atualizando candidatos encaminhados...<br /><br /><i class='fa fa-spinner fa-pulse fa-3x fa-fw font-blue01'></i></center></td></tr></table>");
...
function atualizaListaBaixasAutomaticas(id_vaga) {
    ...
    $("#tabela_baixas_automaticas_div").html("<table width='100%' class='tabela_encaminhados'><tr class='table_formacao_cab'><td>&nbsp;</td></tr><tr class='table_formacao_row'><td style='height: 300px;'><center>Atualizando baixas autom·ticas...<br /><br /><i class='fa fa-spinner fa-pulse fa-3x fa-fw font-blue01'></i></center></td></tr></table>");
    $("#tabela_baixas_automaticas_div_pesq").html("<table width='100%' class='tabela_encaminhados'><tr class='table_formacao_cab'><td>&nbsp;</td></tr><tr class='table_formacao_row'><td style='height: 300px;'><center>Atualizando baixas autom·ticas...<br /><br /><i class='fa fa-spinner fa-pulse fa-3x fa-fw font-blue01'></i></center></td></tr></table>");
...
function atualizaListaPreSelecionados(id_vaga) {
    ...
    $("#tabela_pre_selecionados_div").html("<table width='100%' class='tabela_encaminhados'><tr class='table_formacao_cab'><td>&nbsp;</td></tr><tr class='table_formacao_row'><td style='height: 300px;'><center>Atualizando candidatos prÈ-selecionados...<br /><br /><i class='fa fa-spinner fa-pulse fa-3x fa-fw font-blue01'></i></center></td></tr></table>");
    $("#tabela_pre_selecionados_div_pesq").html("<table width='100%' class='tabela_encaminhados'><tr class='table_formacao_cab'><td>&nbsp;</td></tr><tr class='table_formacao_row'><td style='height: 300px;'><center>Atualizando candidatos prÈ-selecionados...<br /><br /><i class='fa fa-spinner fa-pulse fa-3x fa-fw font-blue01'></i></center></td></tr></table>");
...
function atualizaListaContratados(id_vaga) {
    ...
    $("#tabela_contratados_div").html("<table width='100%' class='tabela_encaminhados'><tr class='table_formacao_cab'><td>&nbsp;</td></tr><tr class='table_formacao_row'><td style='height: 300px;'><center>Atualizando candidatos contratados...<br /><br /><i class='fa fa-spinner fa-pulse fa-3x fa-fw font-blue01'></i></center></td></tr></table>");
    $("#tabela_contratados_div_pesq").html("<table width='100%' class='tabela_encaminhados'><tr class='table_formacao_cab'><td>&nbsp;</td></tr><tr class='table_formacao_row'><td style='height: 300px;'><center>Atualizando candidatos contratados...<br /><br /><i class='fa fa-spinner fa-pulse fa-3x fa-fw font-blue01'></i></center></td></tr></table>");
...
function atualizaListaDispensados(id_vaga) {
    ...
    $("#tabela_dispensados_div").html("<table width='100%' class='tabela_encaminhados'><tr class='table_formacao_cab'><td>&nbsp;</td></tr><tr class='table_formacao_row'><td style='height: 300px;'><center>Atualizando candidatos dispensados...<br /><br /><i class='fa fa-spinner fa-pulse fa-3x fa-fw font-blue01'></i></center></td></tr></table>");
    $("#tabela_dispensados_div_pesq").html("<table width='100%' class='tabela_encaminhados'><tr class='table_formacao_cab'><td>&nbsp;</td></tr><tr class='table_formacao_row'><td style='height: 300px;'><center>Atualizando candidatos dispensados...<br /><br /><i class='fa fa-spinner fa-pulse fa-3x fa-fw font-blue01'></i></center></td></tr></table>");
...
function atualizaContratado(id_vagacandidato, nm_candidato, ds_email) {
    ...
    $("#img_contratar_" + id_vagacandidato).hide();
    $("#img_contratado_" + id_vagacandidato).fadeIn(1000);
...
function escondeCNH() {
    //esconde o cnh a idade na vaga seja escolhida a opÔøΩÔøΩo atÔøΩ 16 ou atÔøΩ 18, caso contrÔøΩrio mostra.
    ...
    if (document.getElementById('ds_idade').value === "16") {
        ...
        //Seleciona o index 0, onde estÔøΩ marcado no select como vazio.
...
function verifica_vazio_profissoes() {
    if (document.getElementById('profissoes').value === "") {
        alert("Campo em branco, escolha uma ou mais profissıes para concorrer as vagas do banco de oportunidades");
        return false;
    }
}
